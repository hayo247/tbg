...
var	item2 = [[15000, 18000, 20000, 20000, 20000, 20000]
			,[18000, 21000, 21000, 21000, 21000, 21000]
			,[20000, 24000, 24000, 24000, 24000, 24000]];
var selItem = [];
...
$(function(){
	...
	$("#glasstype_select").change(function(){
		...
		imgSrc = defImgSrc + "build/" + $("#glasstype_select option:checked").text() + "/" +  $("#ul_glassSort .selected a").text() + ".jpg";
	...
	$('#glasssize_width, #glasssize_height, #glasssize_count').on('keyup', function(){			
		 ...
		 * 그외 : 1500 * 2100
		 */
		if($("#glasstype_select").val() != 0){
			if(parseInt($('#glasssize_width').val().replace(/[^0-9]/, '')) > 1500){
				layer_popup($("#layer_alert"), "A", '선택하신 거울의 최대 가로 폭은 1500mm 를 초과하실 수 없습니다.');
				$('#glasssize_width').val("1500");
			}			
		}
		
		if(parseInt($('#glasssize_height').val().replace(/[^0-9]/, '')) > 2100){
			layer_popup($("#layer_alert"), "A", '선택하신 거울의 최대 길이는 2100mm 를 초과하실 수 없습니다.');
			$('#glasssize_height').val("2100");
		}
...
function initStatus(type){
	...
	$("#build_select option:eq(0)").prop('selected', true);
}
...
function set_cal_price(){
	...
	if($("#glasstype_select").val() != '' && $('#glasssize_width').val().replace(/[^0-9]/, '') != '' && $('#glasssize_height').val().replace(/[^0-9]/, '') != ''){
		var type = $("#ul_glassSort .selected input").val();

		if(15 <= totalJa){
			if("01" == type){
				input_price = parseInt(item2[0][$("#glasstype_select").val()]);
			} else if("02" == type || "03" == type){
				input_price = parseInt(item2[1][$("#glasstype_select").val()]);
			} else{
				input_price = parseInt(item2[2][$("#glasstype_select").val()]);
			}
			...
			if("01" == type){
				input_price = parseInt(item[0][$("#glasstype_select").val()]);
			} else {
				input_price = parseInt(item[1][$("#glasstype_select").val()]);
			}
...
function addCart(){	
	...
	html += "	<input type='hidden' name='build' value='" + $("#build_select option:checked").text() + "'/>";
	...
	if(isMobile() && !$(".fix_head_table thead").is(':visible')){
		...
		html += "				<span class='td_value'>" + $("#glasstype_select option:checked").text() + " / <br>" + $('input[name=chk_sheet]:checked').val() + "</span>";
		html += "			</li><li>";
		if("" != $("#build_select option:checked").val()){
			html += '				<span class="td_title td_plus"> + 시공</span>';		
			html += "				<span class='td_value'>" + $("#build_select option:checked").text() + "</span>";
			html += "			</li><li>";
		}
		...
		html += "	<td class='glasstype'>" + $("#glasstype_select option:checked").text() + "/<br>" + $('input[name=chk_sheet]:checked').val() + "</td>";
		
		if("" != $("#build_select option:checked").val()){
			html += "	<td class='glasssheet'>" + $("#build_select option:checked").text() + "</td>";
		}else{
			html += "	<td class='glasssheet'> - </td>";
		}
...
function send_email(){
	...
	$('#glass_cal_cart > tbody tr').each(function(index){
		...
		cartTxt += $(this).find('input[name="build"]').val() + "|" ;
	...
	$.ajax({
		data : queryString,
		type : 'post',
		url : 'https://script.google.com/macros/s/AKfycbx7lgTloo_1OghtHRwToU3ks2qQ0n6nP2RxI_wHjq2bqlo9kfccmrwgjHapBlhsm88FWQ/exec',
		dataType : 'json',
